...
If FileN = "LJX弹窗杀手控制面板.exe" Then
    Call MsgBox("不支持拦截LJX弹窗杀手控制面板.exe！", vbOKOnly + vbCritical, "不支持拦截的程序")
    Text1.Text = ""
    Text2.Text = ""
    FileN = ""
End If
...
If a = True Then
    Call MsgBox("进程" & Stri & "已经被启动。", vbOKOnly + vbInformation, "进程" & Stri & "已启动")
Else
...
TestFileName (t)
TestFileName (FileN)
Open ("C:\Users\" & MyName & "\AppData\Roaming\LJXPopKiller\Inf\Pops\" & MaxPops & ".ltx") For Output As #1
Print #1, Text1.Text
MaxPops = MaxPops + 1
Close #1
Call LoadAll
